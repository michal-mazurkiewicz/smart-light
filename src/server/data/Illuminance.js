...
const setLightIlluminance = (lightIndex, value) => {IlluminanceArray[lightIndex + 1] = lightIlluminanceArray[lightIndex].map(l => l * (value / 255))}

const setLightsIlluminance = (lightsData) => {
    lightsData.forEach((l, i) => setLightIlluminance(i, l.bottom))
}
...
const getUnderLightAverages = () => {averages = [
  [...totalIlluminance.slice(0, 5), ...totalIlluminance.slice(10, 15), ...totalIlluminance.slice(20, 25)].reduce((p,c) => p + c) / 15,
  [...totalIlluminance.slice(5, 10), ...totalIlluminance.slice(15, 20), ...totalIlluminance.slice(25, 30)].reduce((p,c) => p + c) / 15,
  ...
  [...totalIlluminance.slice(70, 75), ...totalIlluminance.slice(80, 85), ...totalIlluminance.slice(90, 95)].reduce((p,c) => p + c) / 15,
  [...totalIlluminance.slice(75, 80), ...totalIlluminance.slice(85, 90), ...totalIlluminance.slice(95,100)].reduce((p,c) => p + c) / 15
]}

const setSensorIlluminance = (sensor) => {
    if(sensor.name === 'Sensor 1'){
        setSensorOneIlluminance(sensor.illumminance)}
    else{
        setSensorTwoIlluminance(sensor.illumminance)
    }
}

const getRoomIlluminanceAvg =  (sensor, lightsData) => {
    setSensorIlluminance(sensor);
    mergeIlluminanceArrays();
    setLightsIlluminance(lightsData);
    getTotalIlluminance();
    getUnderLightAverages();

    average = averages.reduce((p,n) => p + n) / averages.length
